...
function getThumbnailArray() {
    const $thumbnails = document.querySelectorAll('[data-image-role="trigger"]');
    // console.log($thumbnails); <----- debugging
    return [...$thumbnails];
    // const thumbsArray = [...$thumbnails];
    // return thumbsArray;
}

// 상세 이미지와 제목을 변경해주는 함수
function setDetails(thumbnail) {
    const imgUrl = thumbnail.dataset.imageUrl;
    const title = thumbnail.dataset.imageTitle;
...
function randomFirstDetail(thumbnails) {
    ...
    // console.log(thumbnails[rn]); <----- debugging

    // 랜덤 썸네일을 detail-image 영역에 배치
